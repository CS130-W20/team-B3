...
class Location(models.Model): # Various locations are stored in this table, including those for dining halls, home locations of users, and (later) current locations of sellers
...
class Account(User):
    ...
    pw = models.CharField(max_length=255) # Frontend hashes the password, alternatively we can use Django authentication
    phone = models.CharField(max_length=30)  # Doesn't make sense to use numerical, we'll need to validate on the front-end though
...
class Swipe(models.Model):
    ...
    status = models.CharField(max_length=1, choices=SWIPE_STATES, default=0) # Gets set to 1 after a buyer/seller pairing has occured, 2 when the buyer/seller meet up and confirm, 3 if the seller didn't have the swipe and a refund needs to happen
